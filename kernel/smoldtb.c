...
#define SMOLDTB_STATIC_BUFFER_SIZE 8192
 ...
 */
static uint64 *kernel_dtb;
uint64 * dtb_address(void)
{
    return kernel_dtb;
}
struct fdt_header
...
void DTB_init(void)
{
    ...
    if (!dtb_init(dtb_address(), ops))
        panic("Failed to initialize Device Tree");
}
